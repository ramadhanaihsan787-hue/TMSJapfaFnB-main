...
export default function Dashboard() {
    ...
    const [kpiData, setKpiData] = useState({
        totalShipments: 0,
        otifRate: "0%",
        rejectionRate: "0%", // 🌟 GW UBAH JADI 0% (MURNI DATA REAL NANTINYA)
        totalWeightKg: "0" 
    });

    // 🌟 STATE BARU BUAT LAPIS 2
    ...
    useEffect(() => {
        ...
        // 3. Narik Fleet Utilization (Grafik Donat)
        ...
        // 🌟 5. Narik Data Rejection
        fetch('http://localhost:8000/api/dashboard/rejections')
            .then(res => res.json())
            .then(data => {
                if (data.status === "success") setRejections(data.data);
            })
            .catch(err => console.error("Gagal narik Rejections:", err));
        ...
        fetch('http://localhost:8000/api/dashboard/alerts')
            ...
            .catch(err => console.error("Gagal narik Alerts:", err))
    ...
    const getBarHeight = (count: number) => {
        if (count === 0) return "5%"; // Biar ngga ilang banget kalau 0
    ...
    return (
        <>
            ...
            <div className="p-4 md:p-8 flex flex-col gap-6 md:gap-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                    ...
                    <div className="bg-white dark:bg-[#1a1a1a] p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
                        ...
                        <div className="flex items-baseline gap-2">
                            ...
                            {/* 🌟 CUMA NAMPILIN PANAH KALAU DATANYA BUKAN 0% */}
                            {kpiData.rejectionRate !== "0%" && (
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="w-full lg:w-[70%] bg-white dark:bg-[#1a1a1a] p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
                        ...
                        <div className="h-[250px] flex items-end gap-4 px-4">
                            {isLoading ? (
                                <div className="w-full h-full flex justify-center items-center font-bold text-slate-400">Menghitung Jam Sibuk...</div>
                            ) : volumeData.length === 0 ? (
                        ...
                        <div className="relative flex-1 flex items-center justify-center">
                            {isLoading ? (
                                ...
                                <div className="relative w-48 h-48">
                                    {/* SVG Donut Asli yang udah Dinamis */}
                ...
                {/* 🌟 LAPIS BAWAH: ALERT & REJECTION MURNI DATA REAL DARI API (BUKAN DUMMY LAGI!) */}
                    ...
                    <div className="bg-white dark:bg-[#1a1a1a] p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
                        ...
                        <div className="space-y-6">
                            {isLoading ? (
                                ...
                                // 🌟 KALAU KOSONG, TAMPILIN INI (MURNI BERSIH)
                                <span className="font-bold text-slate-400 text-sm">Belum ada laporan barang diretur hari ini.</span>
                            ) : (
                                // 🌟 KALAU ADA DATA, BARU DI-LOOP (NGEGANTIIN 3 DIV DUMMY LU KEMAREN)
                    ...
                    <div className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col h-[320px] transition-colors">
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-2">
                            {isLoading ? (
                                ...
                                // 🌟 SAMA KAYAK REJECTION, INI NGEGANTIIN 4 DIV ALERT DUMMY LU
                ...
                <div className="mt-4 bg-white dark:bg-[#1a1a1a] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-[85vh] min-h-[600px]">
                    ...
                    <div className="flex-1 relative bg-slate-100 dark:bg-[#0a0a0a]">
                        ...
                        >
                            ...
                            {!isLoading && activeTrucks.map((truck, idx) => (
                                <Marker
                                    key={idx}
