...
export default function Analytics() {
    // 🌟 FIX: MESIN WAKTU BARU! (Dari Date - Sampai Date)
    const [startDate, setStartDate] = useState(() => {
        const today = new Date();
        const lastMonth = new Date(today);
        lastMonth.setDate(today.getDate() - 30); // Default ke 30 hari ke belakang
        return lastMonth.toISOString().split('T')[0];
    });
    
    const [endDate, setEndDate] = useState(() => new Date().toISOString().split('T')[0]);

    // 🌟 URL API SEKARANG DINAMIS NGIKUTIN 2 TANGGAL
    const queryString = `?startDate=${startDate}&endDate=${endDate}`;
    
    const { data: summary, loading: summaryLoading, execute: fetchSummary } = useApi<{ data: KPISummary }>(`${API_ENDPOINTS.ANALYTICS}/kpi-summary${queryString}`);
    const { data: utilization, loading: utilizationLoading, execute: fetchUtilization } = useApi<{ data: FleetUtilization }>(`${API_ENDPOINTS.ANALYTICS}/fleet-utilization${queryString}`);
    const { data: volume, loading: volumeLoading, execute: fetchVolume } = useApi<{ data: DeliveryVolume[], max: number }>(`${API_ENDPOINTS.ANALYTICS}/delivery-volume${queryString}`);
    const { data: drivers, loading: driversLoading, execute: fetchDrivers } = useApi<{ data: DriverPerformance[] }>(`${API_ENDPOINTS.ANALYTICS}/driver-performance${queryString}`);

    // 🌟 EFEK BAKAL JALAN ULANG TIAP KALI TANGGAL BERUBAH
    useEffect(() => {
        if (startDate && endDate) {
            // Validasi simpel biar tanggal mulai ngga kelewat dari tanggal akhir
        ...
        <div className="flex-1 overflow-auto bg-slate-50 dark:bg-[#0A0A0A]">
            <header className="sticky top-0 z-10 bg-white dark:bg-[#111111] border-b border-slate-200 dark:border-[#333] px-4 md:px-8 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4 w-full lg:w-auto">
                    ...
                    {/* 🌟 FILTER RENTANG TANGGAL (CUSTOM DATE PICKER) */}
