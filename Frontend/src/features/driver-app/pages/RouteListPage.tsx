    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#1a1c1e] font-sans transition-colors duration-300">
            ...
            <main className="max-w-md mx-auto px-4 py-6 space-y-4 pb-24">
                ...
                {!isLoading && tripData?.stops.map((stop: any) => {
                    
                    // 1. UI KARTU SELESAI
                    if (stop.status === 'completed') {
                        return (
                            <div key={stop.id} className="bg-slate-100 dark:bg-[#2c2e33]/50 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 opacity-60">
                    ...
                    // 2. UI KARTU AKTIF (Tujuan Berikutnya)
                    if (stop.status === 'active') {
                        return (
                            <div key={stop.id} className="bg-white dark:bg-[#2c2e33] rounded-3xl p-6 border-2 border-primary shadow-xl shadow-primary/10 relative overflow-hidden transition-all active:scale-[0.98]">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex gap-3">
                                            <button
                                                onClick={() => { viewStopDetail(stop); navigate('/driver/navigation'); }}
                                            ...
                                            >
                                                <span className="material-symbols-outlined text-lg">navigation</span> Navigate
                                            </button>
                                            <button
                                                onClick={() => { viewStopDetail(stop); navigate('/driver/detail'); }}
                                            ...
                                            >
                                                <span className="material-symbols-outlined text-lg">info</span> Details
                                            </button>
                                        </div>
                                    </div>
                                </div>
                    ...
                    // 3. UI KARTU PENDING
                    return (
                        <div key={stop.id} onClick={() => { viewStopDetail(stop); navigate('/driver/detail'); }} className="bg-white dark:bg-[#2c2e33] rounded-3xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:bg-slate-50 dark:hover:bg-slate-800/50 active:scale-[0.98] cursor-pointer">
                            <div className="flex items-start gap-4">
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-[#2c2e33] border-t border-slate-100 dark:border-slate-800 flex justify-between items-center px-10 py-3 pb-safe z-50">
                ...
                <div className="flex flex-col items-center gap-1 text-slate-400 cursor-pointer hover:text-primary" onClick={() => navigate('/driver/summary')}>
                    <span className="material-symbols-outlined">history</span>
                    <span className="text-[10px] font-bold uppercase tracking-widest">History</span>
                </div>
                <div className="flex flex-col items-center gap-1 text-slate-400 cursor-pointer hover:text-primary">
