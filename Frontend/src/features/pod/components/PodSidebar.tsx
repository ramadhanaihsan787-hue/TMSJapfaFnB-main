...
export default function PodSidebar() {
    ...
    return (
        <aside className="w-72 bg-white dark:bg-[#111111] text-slate-600 dark:text-slate-400 border-r border-slate-200 dark:border-white/5 flex flex-col justify-between shrink-0 z-10 transition-colors">
            ...
            {/* Profile Dropdown Area (Sama Persis kayak yg lu kirim) */}
            <div className="relative">
                    ...
                    <div className="absolute bottom-[calc(100%+12px)] left-4 w-64 bg-white dark:bg-[#1F1F1F] rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden flex flex-col z-50">
                        ...
                        <div className="p-4">
                            ...
                            <div className="space-y-1">
                                <button onClick={() => navigate('/logistik')} className="w-full flex items-center px-3 py-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all">
                                    <div className="flex items-center gap-3"><span className="material-symbols-outlined text-xl">admin_panel_settings</span><span className="text-sm font-medium">Admin Logistik</span></div>
                                </button>
                                <button onClick={() => navigate('/manager')} className="w-full flex items-center px-3 py-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all">
                                    <div className="flex items-center gap-3"><span className="material-symbols-outlined text-xl">monitoring</span><span className="text-sm font-medium">Manager Logistik</span></div>
                                </button>
                                <button onClick={() => navigate('/pod')} className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl bg-primary/10 text-primary group transition-all">
                                    <div className="flex items-center gap-3"><span className="material-symbols-outlined text-xl">receipt_long</span><span className="text-sm font-bold">Admin POD</span></div>
                                    <span className="material-symbols-outlined text-xl">check_circle</span>
                                </button>
                                <button onClick={() => navigate('/driver')} className="w-full flex items-center px-3 py-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all">
                                    <div className="flex items-center gap-3"><span className="material-symbols-outlined text-xl">local_shipping</span><span className="text-sm font-medium">Driver</span></div>
                                </button>
                            </div>
                        </div>
                        {/* Logout */}
                        <div className="p-2 bg-slate-50 dark:bg-black/20 mt-2">
                            <button onClick={() => { logout(); navigate('/login'); }} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-400/10 hover:text-red-600 dark:hover:text-red-300 transition-all">
                                <span className="material-symbols-outlined text-xl">logout</span><span className="text-sm font-bold">Logout</span>
                            </button>
                        </div>
                    </div>
                )}
                ...
                {/* Profile Toggle */}
                <div className="p-4 bg-slate-50 dark:bg-white/5 border-t border-slate-200 dark:border-white/10 m-4 rounded-xl cursor-pointer hover:bg-slate-100 dark:hover:bg-white/10 transition-colors group" onClick={() => setIsProfileOpen(!isProfileOpen)}>
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-slate-200 dark:bg-slate-700 border border-slate-300 dark:border-white/20 flex flex-col items-center justify-center font-bold text-slate-500 dark:text-slate-300">AP</div>
                        <div className="flex-1 min-w-0">
                        ...
                        <button className="text-slate-500 group-hover:text-primary transition-colors"><span className="material-symbols-outlined text-xl">swap_horiz</span></button>
                    </div>
                </div>
            </div>
        </aside>
