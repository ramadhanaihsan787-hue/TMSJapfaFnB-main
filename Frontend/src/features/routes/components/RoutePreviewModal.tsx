...
interface RoutePreviewModalProps {
    previewData: any; 
    truckColors: string[];
    zoningData?: any; // Udah ngga dipake buat kotak, tapi tetep ada biar ngga error props
    onCancel: () => void;
    onProceedDispatch: (draft: any) => void;    
    onResequence?: (draftData: any) => Promise<any>; 
}

// 🌟 FUNGSI SAKTI: Bikin kotak zona langsung di Frontend!
const getConvexHull = (points: number[][]) => {
    if (points.length <= 3) return points;
    let p0 = points[0];
    for (let i = 1; i < points.length; i++) {
        if (points[i][1] < p0[1] || (points[i][1] === p0[1] && points[i][0] < p0[0])) p0 = points[i];
    }
    const polarAngle = (p: number[]) => Math.atan2(p[1] - p0[1], p[0] - p0[0]);
    const sorted = points.slice().sort((a, b) => {
        const angleA = polarAngle(a);
        const angleB = polarAngle(b);
        if (angleA === angleB) return (a[0]-p0[0])**2 + (a[1]-p0[1])**2 - ((b[0]-p0[0])**2 + (b[1]-p0[1])**2);
    ...
    for (const p of sorted) {
        while (hull.length >= 2) {
            ...
            if ((p2[0] - p1[0]) * (p[1] - p1[1]) - (p2[1] - p1[1]) * (p[0] - p1[0]) <= 0) hull.pop();
            else break;
        }
        hull.push(p);
    }
...
export default function RoutePreviewModal({ previewData, truckColors, onCancel, onProceedDispatch, onResequence }: RoutePreviewModalProps) {
    ...
    // 🌟 FIX WARNA KETUKER: Gambar kotaknya nyesuain isi rute secara Real-Time!
    const dynamicZoningGeoJSON = useMemo(() => {
        if (!draftData?.jadwal_truk_internal) return null;
        const features: any[] = [];
        draftData.jadwal_truk_internal.forEach((truk: any, i: number) => {
            const color = truckColors[(truk.color_index ?? i) % truckColors.length];
            const points: number[][] = [];
            truk.detail_perjalanan.forEach((stop: any) => {
                if (stop.lon && stop.lat && stop.nama_toko !== "📍 GUDANG JAPFA" && stop.keterangan !== "Start" && stop.keterangan !== "Finish") {
                    points.push([stop.lon, stop.lat]);
                }
            });
            if (points.length >= 3) {
                const hull = getConvexHull(points);
                const polyCoords = [...hull, hull[0]]; // Tutup garisnya
                features.push({
                    type: 'Feature',
            ...
            <div className="bg-white dark:bg-[#111] flex-1 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95">
                ...
                <div className="flex-1 relative bg-slate-100 dark:bg-[#0a0a0a]" style={{ minHeight: '600px' }}>
                    <Map ref={mapRef} {...viewState} onMove={(e: any) => setViewState(e.viewState)} style={{ width: '100%', height: '100%' }} mapStyle="mapbox://styles/mapbox/dark-v11" mapboxAccessToken={import.meta.env.VITE_MAPBOX_TOKEN}>
                        ...
                        {dynamicZoningGeoJSON && (
                            <Source id="zoning-preview" type="geojson" data={dynamicZoningGeoJSON as any}>
                                    ...
                                    {truk.detail_perjalanan.map((stop: any, j: number) => {
                                        ...
                                        return (
                                            <Marker key={`${i}-${j}`} longitude={stop.lon} latitude={stop.lat} anchor="center">
                                                ...
                                                >
                                                    {/* 🌟 FIX URUTAN: Pake index Loop Real-time */}
                                                    {j}
                                                </div>
                                            </Marker>
                                        )
