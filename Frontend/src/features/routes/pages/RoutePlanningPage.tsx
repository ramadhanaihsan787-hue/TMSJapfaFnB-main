...
export default function RoutePlanning() {
    ...
    // 🌟 1. FUNGSI FETCH ROUTES (DENGAN TOKEN)
    ...
    // 🌟 2. FUNGSI UPLOAD EXCEL (DENGAN TOKEN)
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('http://localhost:8000/api/orders/upload', { 
                method: 'POST', 
                headers: { 'Authorization': `Bearer ${token}` },
                body: formData 
            });
    ...
    // 🌟 3. FUNGSI UBAH WAKTU (DENGAN TOKEN)
    ...
    // 🌟 4. FUNGSI SAVE KOORDINAT MANUAL (DENGAN TOKEN)
    ...
    // 🌟 5. FUNGSI TEMBAK AI VRP (DENGAN TOKEN)
    const handleOptimizeRoute = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('http://localhost:8000/api/routes/optimize?preview=true', { 
                method: 'POST', 
                headers: { 
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    // 🌟 6. FUNGSI SAVE RUTE PERMANEN (DENGAN TOKEN)
