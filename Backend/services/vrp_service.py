    ...
    def cluster_first_pipeline(pending_orders: List[Any], vehicles: List[Any], settings: Any):
        ...
        try:
            ...
            for key, orders in grouped_orders.items():
                ...
                locations.append({
                    "node_id": node_counter, # 🌟 FIX TOKO HILANG: Simpan ID buat referensi!
            ...
            # Hitung jumlah truk rasional
            ideal_trucks = int((total_berat_semua // 2500) + 1)
            active_count = min(ideal_trucks, len(vehicles))
            if active_count < 1: active_count = 1
            
            logger.info(f"🚚 Mempersiapkan {active_count} Armada untuk total muatan {total_berat_semua}kg")
            ...
            def _solve_zone_worker(cluster_idx, cluster_stores):
                ...
                from utils.helpers import classify_store
                zone_is_mall = [False] + [classify_store(s['nama_toko']) for s in cluster_stores]
                
                start_min = 360; end_min = 1200 
                zone_tw = [(start_min, end_min)] * len(zone_locs)
    ...
    def match_routes_to_fleet(final_routes_data, vehicles, settings, node_to_orders, spillover_basket):
        ...
        for rd in final_routes_data:
            ...
            extracted_routes.append({
                "cluster_idx": rd["cluster_idx"], # 🌟 FIX WARNA: Bawa Index Zona Asli ke Frontend
        ...
        for i, r_data in enumerate(extracted_routes):
            ...
            if r_data["weight"] > (assigned_vehicle.capacity_kg * (settings.vrp_capacity_buffer_percent / 100.0)):
                logger.warning(f"Truk {assigned_vehicle.license_plate} OVERLOAD! Memotong rute...")
            ...
            manifest = []
            current_time = 420 # 07:00 Pagi default
            prev_node = 0
            global_seq = 0 # Counter urutan global biar rapi
            ...
            for step, node_idx in enumerate(r_data["route_indices"]):
                ...
                if node_idx == 0:
                    ...
                    service_time = settings.vrp_base_drop_time_mins
                    current_time += r_data["time_mat"][prev_node][node_idx]
                    
                    # 🌟 FIX TOKO HILANG: Tarik ID DO (order_id) dari node_to_orders!
                    if original_node_id and original_node_id in node_to_orders:
                        ...
                        for sub_idx, order in enumerate(orders_in_node):
                            manifest.append({
                                "urutan": global_seq, 
                                "nomor_do": order.order_id, # INI KUNCI BIAR TOKO NONGOL DI DETAIL PANEL
            ...
            formatted_routes.append({
                "route_id": route_id, 
                "color_index": r_data["cluster_idx"], # 🌟 FIX WARNA
