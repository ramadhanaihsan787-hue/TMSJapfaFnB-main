...
def get_returns_dashboard(db: Session):
    ...
    fleet_performance = [{"plate": k, "count": v["count"], "weight": round(v["weight"], 1), "trend": "up", "percent": "+2%"} for k, v in fleet_map.items()]
...
def get_efficiency_dashboard(db: Session, settings):
    ...
    for idx, rute in enumerate(rutes[:5]):
        ...
        op_excellence.append({
            "route": f"Route #{rute.route_id}", "region": "Jabodetabek Hub", "otif": "95%", "lead": "4.2h", "factor": f"{lf_rute}%",
    ...
    return {
        "kpi": {
            "totalShipments": total_shipments, "avgLeadTime": "4.2h", "loadFactor": f"{lf_percent}%",
            "costPerKg": f"Rp {cost_per_kg}", "hiddenCost": "4.2%"
        },
        "lfTrend": lf_trend,
        "costDist": cost_dist,
