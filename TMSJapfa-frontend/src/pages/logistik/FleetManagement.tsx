...
export default function FleetManagement() {
    ...
    // 🌟 STATE UNTUK DATA SUNTIKAN BACKEND
    const [fleetList, setFleetList] = useState<any[]>([]);
    const [selectedTruck, setSelectedTruck] = useState<any>(null);
    const [isLoading, setIsLoading] = useState(true);

    // 🌟 NARIK DATA ARMADA DARI API
    useEffect(() => {
        fetch('http://localhost:8000/api/fleet')
            ...
            .catch(err => {
                console.error("Gagal narik data armada:", err);
                setIsLoading(false);
            });
    }, []);

    // Simulasi Suhu Dinamis berdasarkan Truk yang dipilih
    const mockTemp = selectedTruck ? (2.1 + (selectedTruck.id % 3) * 0.5).toFixed(1) : "2.4";
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 md:space-y-8 custom-scrollbar">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                        <div className="bg-white dark:bg-[#111111] p-6 rounded-xl border border-slate-200 dark:border-[#333] shadow-sm">
                            ...
                            <div className="flex items-end justify-between mt-2">
                                <h3 className="text-2xl font-bold text-[#111] dark:text-white">
                                    {isLoading ? '...' : fleetList.filter(t => t.status === 'Available').length} 
                                    <span className="text-base font-normal text-slate-400 ml-1">/ {fleetList.length || 7}</span>
                                </h3>
                        ...
                        <div className="bg-white dark:bg-[#111111] p-6 rounded-xl border border-slate-200 dark:border-[#333] shadow-sm">
                            ...
                            <div className="flex items-end justify-between mt-2">
                                <h3 className="text-2xl font-bold text-[#111] dark:text-white">
                                    {isLoading ? '...' : fleetList.filter(t => t.status === 'Maintenance').length || 1} 
                                </h3>
                            ...
                            <table className="w-full text-left min-w-[800px] border-collapse">
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-[#333]">
                                    {isLoading ? (
                                        ...
                                        // DUMMY FALLBACK SEBELUM API NYALA
                                        <>
                                            <tr onClick={() => setSelectedTruck({id: 1, plateNumber: "B 9044 JXS", model: "Isuzu ELF", status: "Available"})} className="hover:bg-slate-50 dark:hover:bg-[#1a1a1a] transition-colors cursor-pointer bg-primary/5">
                                                <td className="px-6 py-4 font-bold text-primary">TRK-01</td>
                                                <td className="px-6 py-4 text-sm font-bold text-[#111] dark:text-white">Isuzu ELF</td>
                                                <td className="px-6 py-4 text-sm font-bold text-[#111] dark:text-white">B 9044 JXS</td>
                                                <td className="px-6 py-4 text-sm text-slate-500">45,200</td>
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="text-xs font-bold text-[#111] dark:text-white">12.0</span>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className="px-3 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-[10px] font-bold uppercase rounded-full tracking-wide border border-emerald-200 dark:border-emerald-800">Available</span>
                                                </td>
                                            </tr>
                                            <tr onClick={() => setSelectedTruck({id: 2, plateNumber: "B 9513 JXS", model: "Hino Dutro", status: "In Route"})} className="hover:bg-slate-50 dark:hover:bg-[#1a1a1a] transition-colors cursor-pointer">
                                                <td className="px-6 py-4 font-bold text-[#111] dark:text-slate-300">TRK-02</td>
                                                <td className="px-6 py-4 text-sm font-bold text-[#111] dark:text-white">Hino Dutro</td>
                                                <td className="px-6 py-4 text-sm font-bold text-[#111] dark:text-white">B 9513 JXS</td>
                                                <td className="px-6 py-4 text-sm text-slate-500">89,100</td>
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-24 h-1.5 bg-slate-200 dark:bg-[#333] rounded-full overflow-hidden">
                                                            <div className="bg-primary h-full w-[75%]"></div>
                                                        </div>
                                                        <span className="text-xs font-bold text-[#111] dark:text-white">14.5</span>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-[10px] font-bold uppercase rounded-full tracking-wide border border-blue-200 dark:border-blue-800">In Route</span>
                                                </td>
                                            </tr>
                                        </>
                                    ) : (
                                        ...
                                        fleetList.map((truck, idx) => (
                                            <tr 
                                                ...
                                                className={`hover:bg-slate-50 dark:hover:bg-[#1a1a1a] transition-colors cursor-pointer ${selectedTruck?.id === truck.id ? 'bg-primary/5 dark:bg-primary/10' : ''}`}
                                            >
                                                <td className={`px-6 py-4 font-bold ${selectedTruck?.id === truck.id ? 'text-primary' : 'text-[#111] dark:text-slate-300'}`}>
                                                    TRK-0{truck.id}
                                                </td>
                                                <td className="px-6 py-4 text-sm font-bold text-[#111] dark:text-white">{truck.model}</td>
                                                <td className="px-6 py-4 text-sm font-bold text-[#111] dark:text-white">{truck.plateNumber}</td>
                                                <td className="px-6 py-4 text-sm text-slate-500">{truck.kmAwalHariIni?.toLocaleString() || '0'}</td>
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-24 h-1.5 bg-slate-200 dark:bg-[#333] rounded-full overflow-hidden">
                    ...
                    <div className="flex items-start justify-between">
                        <div>
                            ...
                            <h4 className="text-2xl font-black text-[#111] dark:text-white leading-none mb-1">
                                TRK-0{selectedTruck?.id || '1'}
                            </h4>
                            <p className="text-slate-500 dark:text-slate-400 text-xs font-medium">
                                {selectedTruck?.plateNumber || 'B 9044 JXS'} • {selectedTruck?.model || 'Isuzu ELF'}
                            </p>
                        </div>
