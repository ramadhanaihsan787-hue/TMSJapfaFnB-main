...
# Replace block 1
block1_start_str = '                                    <div className="mt-4 bg-[#111111] rounded-2xl p-4 border border-[#333] shadow-[0_4px_20px_rgba(0,0,0,0.5)] relative overflow-hidden">'
start1 = text.find(block1_start_str)
end1 = text.find('</div>\n\n                                {/* Other Trucks */}', start1)

if start1 != -1 and end1 != -1:
    old_block1 = text[start1:end1+6]
    new_block1 = get_truck('#10b981', 92, 'Optimal • 92%', '1840 / 2000 Kg', 'emerald')
    text = text.replace(old_block1, new_block1, 1)

# Replace block 2
start2 = text.find(block1_start_str)  # Look for the next occurrence since we replaced the first
if start2 != -1:
    # Need to match the closing of the element accurately by counting divs or finding the unique wrapper end
    tmp_text = text[start2:]
    end_marker = '</div>\n                                    </div>\n                                </div>\n                            </div>'
    end_rel = tmp_text.find(end_marker)
    
    if end_rel != -1:
        end2 = start2 + end_rel
        
        # we need to find the correct div close.
        # old_block2 ends with: `</div>\n                                        </div>\n                                    </div>\n                                </div>` (approximately)
        # Let's just find `                                        </div>\n                                    </div>\n                                </div>`
        
        # A more robust regex/replace mechanism
        # Actually we just know it's exactly the div wrapping the 3D Isometric View
        pass

# Since using index parsing is risky, let me just find the two exact strings holding the old truck containers and replace them via regex

import re

# Regex to safely match the exact block 
pattern_truck = re.compile(
    r'<div className="mt-4 bg-\[#111111\] rounded-2xl p-4 border border-\[#333\] shadow-\[0_4px_20px_rgba\(0,0,0,0\.5\)\] relative overflow-hidden">.*?(?=</div>\s*</div>\s*</div>\s*</div>|</div>\s*\{/\* Other Trucks \*/\})',
    re.DOTALL
)
...
if len(matches) >= 2:
    ...
    # Because matches[1] might have a different trailing combination
    text = text.replace(matches[1] + '</div>\n                                        </div>\n                                    </div>', new_block2, 1)

# Quick fallback if regex didn't work (we can check below)
